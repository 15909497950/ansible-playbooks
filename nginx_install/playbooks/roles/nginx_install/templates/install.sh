#!/bin/bash
cd {{ data_path }}
tar zxvf solar-proxy-1.1.tar.gz
cd {{ data_path }}/solar-proxy-1.1
tar zxvf proxy.tar.gz
tar zxvf nginx-1.19.9.tar.gz
cd {{ data_path }}/solar-proxy-1.1
patch -p1 < /deploy/solar-proxy-1.1/ngx_http_proxy_connect_module/patch/proxy_connect_rewrite_1018.patch 
...
--with-ipv6 \
--add-module=/deploy/solar-proxy-1.1/ngx_http_proxy_connect_module 
make && make install 
useradd -s /sbin/nologin -M nginx

#替换ng配置
cp -rf /deploy/solar-proxy-1.1/proxy/*  /etc/nginx/
